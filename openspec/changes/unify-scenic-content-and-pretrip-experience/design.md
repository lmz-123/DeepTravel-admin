...
The “城市故事” workspace first shows cards for every city. Opening one city lists its stories and an add action. The only editable fields are title, public-OSS cover image and story content; city is fixed by the selected card and there is one canonical story kind. The cover control uploads through the shared media endpoint, previews the canonical URL, and stores the resulting media reference without exposing object keys as a normal technical form. A single secondary action saves those values and generates narration with the default published voice. The server resolves/preserves canonical source identity, OSS track, variant, placement and compatibility metadata, so operators never choose a directory item, source kind, content type, track, provider, variant JSON or placement JSON. A migrated legacy home story opens the same minimal editor and updates the same canonical body. Compatibility links translate legacy arc IDs to the unified catalog ID. The old `HomeStoriesWorkspace` is removed from active navigation only after mapping coverage is complete.
...
Public editorial images are normalized at the admin boundary: decode supported image bytes, apply orientation, flatten transparency against a neutral background when necessary, encode JPEG quality 85, then compute checksum/object identity and upload. Audio remains byte-preserving. Existing PNG assets migrate to new immutable JPEG keys; database content references and the `media_assets` row move together transactionally, while original OSS objects remain available for rollback until separately retired.
